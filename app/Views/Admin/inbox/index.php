...
<div class="mb-6 lg:mb-8 mt-2">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl lg:text-3xl font-bold text-slate-800 dark:text-white mb-1">Daftar Pengaduan</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400">Verifikasi dan tindak lanjuti laporan masuk dari
                pelapor.</p>
        </div>
    </div>
</div>

<!-- Stats Card Wrapper (Scrollable on Mobile) -->
<div class="overflow-x-auto pb-4 -mx-4 px-4 lg:mx-0 lg:px-0 hide-scrollbar">
    <div class="flex lg:grid lg:grid-cols-4 gap-4 min-w-[800px] lg:min-w-full">
        <!-- Card 1 -->
        <div
            class="flex-1 bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-4">
            <div
                class="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-xl flex items-center justify-center">
                <span class="material-icons">inbox</span>
            </div>
            <div>
                <h3 class="text-2xl font-bold text-slate-800 dark:text-white"><?= number_format($stats['total'] ?? 0) ?>
                </h3>
                <p class="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Total Laporan</p>
            </div>
        </div>
        <!-- Card 2 -->
        <div
            ...
            <div>
                ...
                <p class="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Belum Dibaca</p>
            </div>
        </div>
        <!-- Card 3 -->
        <div
            class="flex-1 bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-4">
            <div
                class="w-12 h-12 bg-primary/10 dark:bg-primary/20 text-primary rounded-xl flex items-center justify-center">
                <span class="material-icons">hourglass_top</span>
            </div>
            <div>
                <h3 class="text-2xl font-bold text-slate-800 dark:text-white">
                    <?= number_format($stats['diproses'] ?? 0) ?>
                </h3>
                <p class="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Sedang Diproses</p>
            </div>
        </div>
        <!-- Card 4 -->
        <div
            class="flex-1 bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-4">
            <div
                class="w-12 h-12 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-xl flex items-center justify-center">
            ...
            <div>
                ...
                <p class="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Selesai</p>
            </div>
        </div>
    </div>
</div>
...
<div
    ...
    <div class="p-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/20">
        <form action="<?= base_url('admin/laporan-masuk') ?>" method="get">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-5">
                ...
                <div class="lg:col-span-2">
                    <label
                        class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter mb-2 ml-1">Status</label>
                    <div class="relative">
                        <select name="status"
                            ...
                            <option value="0" <?= ($filters['status'] ?? '') == '0' ? 'selected' : '' ?>>Baru
                            </option>
                            <option value="Dibaca" <?= ($filters['status'] ?? '') == 'Dibaca' ? 'selected' : '' ?>>Dibaca
                            </option>
                ...
                <div class="lg:col-span-2">
                    <label
                        class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter mb-2 ml-1">Kategori</label>
                    <div class="relative">
                ...
                <div class="lg:col-span-2">
                    <label
                        class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter mb-2 ml-1">Terlapor</label>
                    <div class="relative">
                        <select name="jenis_terlapor"
                            class="w-full pl-4 pr-10 py-2.5 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-600 dark:text-slate-200 focus:outline-none focus:border-primary appearance-none cursor-pointer font-medium">
                            <option value="">Semua Jenis</option>
                            <option value="Mahasiswa" <?= ($filters['jenis_terlapor'] ?? '') == 'Mahasiswa' ? 'selected' : '' ?>>Mahasiswa</option>
                            <option value="Dosen" <?= ($filters['jenis_terlapor'] ?? '') == 'Dosen' ? 'selected' : '' ?>>
                                Dosen</option>
                            <option value="Tendik" <?= ($filters['jenis_terlapor'] ?? '') == 'Tendik' ? 'selected' : '' ?>>
                                Tendik</option>
                ...
                <div class="lg:col-span-3">
                    <label
                        class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter mb-2 ml-1">Program
                        Studi</label>
                    <div class="relative">
                        <select name="prodi"
                            class="w-full pl-4 pr-10 py-2.5 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-600 dark:text-slate-200 focus:outline-none focus:border-primary appearance-none cursor-pointer font-medium">
                ...
                <div class="lg:col-span-3 flex items-end gap-2">
                    <div class="flex-1">
                        <label
                            class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter mb-2 ml-1">Tanggal</label>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[1000px]">
            <thead
                class="bg-slate-100/50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-bold tracking-widest">
                <tr>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700 text-sm">
                <?php if (!empty($reports)): ?>
                    ...
                    <?php foreach ($reports as $index => $row): ?>
                        <tr
                            class="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors <?= $row['status_read'] ? '' : 'bg-blue-50/20 dark:bg-blue-900/10' ?>">
                            <td class="p-5 text-center font-bold text-slate-400"><?= $startIndex + $index + 1 ?></td>
                            <td class="p-5">
                                <span
                                    class="font-mono font-bold text-primary text-xs bg-primary/10 px-2 py-1 rounded-md">#<?= $row['referensi_pengaduan'] ?></span>
                            </td>
                            <td class="p-5">
                            ...
                            <td class="p-5">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-xl bg-slate-200 flex-shrink-0 relative overflow-hidden shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-primary font-bold uppercase"><?= $row['nm_prodi'] ?? '-' ?>
                                        </p>
                                    </div>
                                </div>
                            </td>
                            <td class="p-5">
                                ...
                                ?>
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl <?= $cat_style ?> text-[11px] font-bold border">
                            ...
                            <td class="p-5">
                                <div class="space-y-1">
                                    ...
                                    <?php if ($row['terlapor_jenis'] == 'Mahasiswa'): ?>
                                        <div class="flex flex-col">
                                            <span class="text-[10px] font-mono font-bold text-slate-500">NIM:
                                                <?= $row['terlapor_nim'] ?: '-' ?>
                                            </span>
                                            <span
                                                class="text-[10px] font-bold text-primary uppercase"><?= $row['nm_prodi'] ?: 'Prodi tidak diketahui' ?></span>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="p-5">
                                <?php
                                $status_map = [
                                    '0' => ['bg' => 'bg-blue-100 text-blue-700', 'dot' => 'bg-blue-500 animate-pulse'],
                                    'dibaca' => ['bg' => 'bg-slate-100 text-slate-700', 'dot' => 'bg-slate-500'],
                                    'diproses' => ['bg' => 'bg-amber-100 text-amber-700', 'dot' => 'bg-amber-500'],
                                    'selesai' => ['bg' => 'bg-green-100 text-green-700', 'dot' => 'bg-green-500'],
                                    'ditolak' => ['bg' => 'bg-red-100 text-red-700', 'dot' => 'bg-red-500'],
                                ];
                                $st = strtolower($row['status_tindakan'] ?? '0');
                                $curr_stat = $status_map[$st] ?? $status_map['0'];
                                ?>
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full <?= $curr_stat['bg'] ?> text-[11px] font-bold">
                                    <span class="w-2 h-2 rounded-full <?= $curr_stat['dot'] ?>"></span>
                                    <?= $row['status_tindakan'] ?: 'Baru' ?>
                                </span>
                            </td>
                    ...
                    <tr>
                        <td colspan="8" class="p-10 text-center">
                            <div class="flex flex-col items-center justify-center text-slate-400">
                                <span class="material-icons text-5xl mb-2 text-slate-200">dashboard_customize</span>
                                <p class="font-bold uppercase tracking-widest text-xs">Belum ada pengaduan masuk</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
    ...
    <!-- Pagination Section -->
    <div
        class="p-6 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-6">
        <div class="text-xs font-bold text-slate-500 uppercase tracking-widest text-center sm:text-left">
            Menampilkan <span class="text-slate-800 dark:text-white"><?= count($reports) ?></span> dari <span
                class="text-slate-800 dark:text-white"><?= $pager->getTotal('default') ?></span> Total Laporan
        </div>
